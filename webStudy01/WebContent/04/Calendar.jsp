...
<form action="">
   <h4>
    <%--   <a href="<%=request.getContextPath() %>/04/calendar.jsp?year=<%= beforeYear%>&month=<%= beforeMonth%>&language=<%= clientLocale.toLanguageTag()%>"> --%>
     <a href="javascript:eventHandler(<%=beforeYear%>,<%=beforeMonth%>);"> 
      이전달
      </a>
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      ...
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
     <%--  <a href="?year=<%= nextYear%>&month=<%= nextMonth%>&language=<%=clientLocale.toLanguageTag()%>"> --%>
     <a onclick="eventHandler(<%=nextYear%>,<%=nextMonth%>);">
      다음달
      </a>
   </h4>
</form>
