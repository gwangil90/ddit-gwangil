 ...
 * 	헤더명 : 헤더값 ....
 * Request Body(Contents, Message body) : method가 POST일 경우에만 형성.
 * 	:클라이언트가 서버로 전송할 데이터들...(요청 파라미터)
 * 
 * ***client의 요청정보를 서버에서 확인하는 방법
 * HttpServletRequest의 확용
...
public class HttpRequestDescription extends HttpServlet {
	...
	//메소드에 상관없이 모든 요청을 발생시키는 콜백함수
	//Request Line에 대한 정보를 뽑아내기 위한 메서드들
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println(request.getCharacterEncoding()); //get메서드여서 boby없기 때문에 null
		System.out.println(request.getContentType()); //get메서드여서 boby없기 때문에 null
		System.out.println(request.getContentLength());
		System.out.println(request.getContextPath()); //webapps경로 
		System.out.println(request.getLocalAddr()); //서버사이드의 ipAdress
		System.out.println(request.getRemoteAddr()); //클라이언트사이드의 ipAdress
		System.out.println(request.getRemotePort()); //클라이언트의 포트넘버
		System.out.println(request.getMethod()); //Http메소드가 돌아온다. Request Line의 정보
