...
<%
	if(StringUtils.isNotBlank(includePage)){
		...
		<%
			if(authMember!=null){
		 %>	
			<%=authMember.getMem_name() %>님 (<%=authMember.getMem_auth() %>),<a href="<%=request.getContextPath()%>/member/mypage.do">마이페이지</a> <a href="<%=request.getContextPath() %>/login/logout.jsp">로그아웃</a>
		<%
			}else{
		%>
